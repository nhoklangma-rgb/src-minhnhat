...
public class ListTichTieu {
  ...
  public static void load() {
    ENTRY.clear();
    try {
      byte[] ab = core.Util.loadfile("data/tich_tieu.json");
      if (ab == null) {
        core.GameLogger.info("data/tich_tieu.json not found!");
        return;
      }
      ...
      for (int i = 0; i < arr.size(); i++) {
        ...
        t.num = Long.parseLong(job.get("num").toString());
      ...
      core.GameLogger.info("Load " + ENTRY.size() + " mốc tích tiêu");
    } catch (Exception e) {
      core.GameLogger.error("ListTichTieu.load: Error loading tich tiêu data", e);
    }
  }
  ...
  public static void process(Player p, Message m2) throws IOException {
    ...
    if (type == 1 && id < p.tichTieuCheck.length && id < ENTRY.size() && p.tichTieuCheck[id] != 1) {
      ...
      ListTichTieu listGet = ENTRY.get(id);
